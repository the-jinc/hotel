...
export default function AdminDashboard() {
  ...
  const { reviews, fetchReviews, loading: reviewsLoading } = useReviewStore();
  ...
  const roomOccupancyData = {
    labels: rooms.slice(0, 5).map((room) => room.type as string),
    datasets: [
      {
        data: rooms.slice(0, 5).map((room) => room.quantity as number),
        backgroundColor: [
          "rgba(79, 70, 229, 0.7)",
          "rgba(99, 102, 241, 0.7)",
